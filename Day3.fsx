...
module Wire =
    ...
    let pointsFrom (start:Point) (instruction:string) =
        let dir = instruction.ToUpper().[0]
        let length = instruction.Substring(1) |> int

        let mapFunc =
            match dir with
            | 'U' ->
                (fun delta -> {start with Y = start.Y - delta})
            | 'D' ->
                (fun delta -> {start with Y = start.Y + delta})
            | 'R' ->
                (fun delta -> {start with X = start.X + delta})
            | 'L' ->
                (fun delta -> {start with X = start.X - delta})
            | _ -> failwithf "Unhandled direction: %c" dir

        // Start at 1 so we don't include the starting point in our
        // results
        [1..length]
        |> List.map mapFunc

    let pointsFromList (instructions:string list) =
        let folder pointsSoFar thisInstruction =
            match List.tryLast pointsSoFar with
            | Some s ->
                // Add points from this instruction to the existing
                // list of points
                let newPoints = pointsFrom s thisInstruction
                pointsSoFar @ newPoints
            | None ->
                // Empty list, so initialize with this instruction
                let newPoints = pointsFrom origin thisInstruction
                newPoints

        instructions
        |> List.fold folder []

    let create (instructions:string) =
        let instructionList = instructions.Split([|','|]) |> Array.toList

        {
            Instructions = instructions
            Points = pointsFromList instructionList
        }

    let intersections wire1 wire2 =
        (wire1.Points |> Set.ofList)
        |> Set.intersect (wire2.Points |> Set.ofList)
        |> Set.toList

    let manhattanDistance point1 point2 =
        // https://en.wikipedia.org/wiki/Taxicab_geometry
        Math.Abs((point1.X - point2.X)) +
            Math.Abs((point1.Y - point2.Y))

    let closestIntersection wire1 wire2 =
        intersections wire1 wire2
        |> List.map (manhattanDistance origin)
        |> List.min

    let stepsToPoint wire point =
        wire.Points
        |> List.findIndex ((=) point)
        |> (+) 1 // Account for zero-based index

    let lowestLatency wire1 wire2 =
        let allIntersections = intersections wire1 wire2

        allIntersections
        |> List.map (fun p ->
            let steps1 = stepsToPoint wire1 p
            let steps2 = stepsToPoint wire2 p
            steps1 + steps2
            )
        |> List.min

module Part1 =
    let getClosest inst1 inst2 =
        let wire1 = Wire.create inst1
        let wire2 = Wire.create inst2

        Wire.closestIntersection wire1 wire2

    let checkTestCases () =
        let check instruction1 instruction2 expected =
            printfn "\nTest case:\n\t%s\n\t%s\n" instruction1 instruction2

            let actual = getClosest instruction1 instruction2

            if expected = actual then
                printfn "OK: %i" expected
            else
                printfn "FAILED: expected %i, got %i" expected actual
                printfn "All intersections:"
                let wire1 = Wire.create instruction1
                let wire2 = Wire.create instruction2
                let allIntersections = Wire.intersections wire1 wire2
                allIntersections
                |> List.map (fun i ->
                    printfn "\t(%i, %i) -> %i"
                        i.X
                        i.Y
                        (Wire.manhattanDistance origin i)
                    )
                |> ignore
                printfn ""
        ...
        check "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83" 159
        check "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51" "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" 135

        printfn ""

    let solve () =
        // Solve part 1
        printfn "Part 1 solution: %i\n" (getClosest puzzleInput1 puzzleInput2)
...
module Part2 =
    let checkTestCases () =
        let check instruction1 instruction2 expected =
            ...
            let actual = Wire.lowestLatency wire1 wire2
            if actual = expected then
                printfn "OK: %i" actual
            else
                printfn "FAILED: expected %i but got %i" expected actual

        check "R8,U5,L5,D3" "U7,R6,D4,L4" 30
        check "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83" 610
        check "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51" "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" 410

        printfn ""

    let solve () =
        // Solve part 2
        let wire1 = Wire.create puzzleInput1
        let wire2 = Wire.create puzzleInput2
        printfn "Part 2 solution: %i\n" (Wire.lowestLatency wire1 wire2)

...
let orbitFromString (str:string) : Orbit =
    let parts = str.Split([|')'|])
    parts.[0], parts.[1]
...
let countOrbits (input:string list) =
    ...
    let orbitCount body =
        let rec orbitCountRec i body =
            let i' = i + 1
            match directMap |> Map.tryFind body with
            | Some s when s = "COM" -> i'
            | Some s -> orbitCountRec i' s
            | None -> failwithf "Could not find body %s in map" body

        orbitCountRec 0 body
